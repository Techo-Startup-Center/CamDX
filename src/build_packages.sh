...
buildInDocker() {
    ...
    docker run --rm -v $XROAD/..:/workspace -w /workspace/src -u builder ${OPT} xroad-build  bash -c "./update_ruby_dependencies.sh && ./compile_code.sh -release" || errorExit "Error running build of binaries."
}

buildLocally() {
    echo "Building locally..."
    cd $XROAD || errorExit "Error 'cd $XROAD'."
    ./compile_code.sh -release || errorExit "Error running build of binaries."
}
